...
- Missing folders are hidden while their database records are retained for recovery. Returning a folder with its identity restores its metadata. Unavailable storage or a failed scan does not delete metadata. Scans run as database transactions; changed source pages invalidate the old SSD snapshot.
